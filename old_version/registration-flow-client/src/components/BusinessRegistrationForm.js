...
import { setBusinessProfileForm } from "../redux/businessProfileFormSlice";
...
const BusinessRegistrationForm = () => {
  ...
  const registerAndNavigate = () => {
    ...
    const registerBusinessPromise = registrationApiClient.registerBusiness(
      businessRegistrationDto,
      (jsonResponse) => {
        // console.log("success!", jsonResponse);
        sessionStorage.setItem("asante:businessId", jsonResponse.business.id);
      ...
      (error) => {
        try {
          ...
          if (errorJson.errors) {
            errorJson.errors.forEach((value, i) => {
              // console.log(`error index ${i}, value: ${JSON.stringify(value)}`);
            ...
            errorJson.message &&
            errorJson.message === "Unauthorized"
          ) {
            navigate(`/`);
            // setUnhandledError(true)
          } else {
          ...
          // console.log("unable to parse error into json");
          setUnhandledError(true);
        }
      },
    );
  ...
  const formIsValid = () => {
    ...
    const newValidationState = {
      ["businessName"]: formData["businessName"].length > 2,
  ...
  const handleChange = (prop) => (event) => {
    ...
    if (prop === "businessName" && nameNotUnique) {
  ...
  const handleBackClick = () => {
    dispatch(setBusinessProfileForm({ selected: formData }));
  ...
  const highlightClass = styles["highlight-business"];
      ...
      >
        ...
        >
          ...
          >
            ...
            <span className={highlightClass}>Business</span>
          </Typography>
        </Box>
        <IconButton
          sx={{
          ...
          <Box width="70%" mb={2}>
            ...
            >
              ...
              <span
                style={{
                  color: "#FF5151",
                  marginTop: "9px",
                  visibility: `${!formValidity.businessName}`,
                  fontSize: "20px",
                  fontWeight: 380,
                }}
              >
                {formValidity.businessName ? "" : " Name is required!"}
              </span>
              <span
                style={{
                  color: "#FF5151",
              ...
              >
                {alreadyRegistered ? " You already belong to a business!" : ""}
              </span>
              <span
                style={{
                  color: "#FF5151",
              ...
              >
                {nameNotUnique ? " Business name already in use!" : ""}
              </span>
              <span
                style={{
                  color: "#FF5151",
            ...
            <TextField
              ...
              InputProps={{
                style: {
                  borderRadius: "9px",
                  borderColor: `${formValidity.businessName ? "#000000" : "red"}`, // Add red border if invalid
                },
                classes: {
