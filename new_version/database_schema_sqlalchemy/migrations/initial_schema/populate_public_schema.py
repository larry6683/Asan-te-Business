...
# Go up 3 levels: initial_schema -> migrations -> database_schema_sqlalchemy
database_schema_root = os.path.dirname(os.path.dirname(current_dir))
print(f"Database schema root: {database_schema_root}")
sys.path.insert(0, database_schema_root)
scripts_path = os.path.join(database_schema_root, 'scripts', 'seed_data')
...
# Try to import seed data function with corrected path
try:
    from public_seed_data import create_seed_data
...
def populate_cause_preference_ranks(session):
    ...
    ranks = [
        CausePreferenceRank(code=1, cause_preference_rank_name='UNRANKED'),      # Changed
        CausePreferenceRank(code=2, cause_preference_rank_name='PRIMARY'),       # Changed
        CausePreferenceRank(code=3, cause_preference_rank_name='SUPPORTING'),    # Changed
        CausePreferenceRank(code=4, cause_preference_rank_name='UNSPECIFIED')    # Added for proto compatibility
    ]
