...
# Add the project root to Python path to import database_schema_sqlalchemy
...
def create_seed_data(database_url="postgresql://asante_dev:password@localhost:5432/postgres"):
    ...
    try:
        ...
        # Get causes
        homelessness_cause = session.query(Cause).filter_by(code=1).first()       # Homelessness 
        disadvantaged_cause = session.query(Cause).filter_by(code=4).first()      # Disadvantaged Populations 
        education_cause = session.query(Cause).filter_by(code=10).first()         # Education 
        social_innovation_cause = session.query(Cause).filter_by(code=14).first() # Innovation 
        drought_cause = session.query(Cause).filter_by(code=17).first()           # Droughts & Fire Management 
        climate_cause = session.query(Cause).filter_by(code=18).first()           # Climate Advocacy 
